...
pub fn generate_nom_read_for_data(
    ...
    quote_spanned! {
        ...
        impl #impl_generics mavryk_data_encoding::nom::NomReader for #name #ty_generics #where_clause {
            fn nom_read(bytes: &[u8]) -> mavryk_data_encoding::nom::NomResult<Self> {
                #nom_read(bytes)
            }
        }
    }
    ...
    if let Some(hash_field) = hash.first() {
        ...
        quote_spanned! {
            hash_field.name.span()=>
                nom::combinator::map(
                    mavryk_data_encoding::nom::hashed(
                        nom::sequence::tuple((
