...
use mavryk_context::initializer::{initialize_tezedge_context, ContextKvStoreConfiguration};
use mavryk_context::{context_key, ContextError, TezedgeContext};
use mavryk_context::{IndexApi, ProtocolContextApi, ShellContextApi};
use mavryk_context_api::{
    ContextKey, MavrykContextTezEdgeStorageConfiguration, MavrykContextTezedgeOnDiskBackendOptions,
};
...
pub fn test_context_set_get_commit_persistent() -> Result<(), anyhow::Error> {
    context_set_get_commit(
        ContextKvStoreConfiguration::OnDisk(MavrykContextTezedgeOnDiskBackendOptions {
            base_path: "".to_string(),
            startup_check: false,
        }),
...
pub fn test_context_set_get_commit() -> Result<(), anyhow::Error> {
    context_set_get_commit(
        ContextKvStoreConfiguration::InMem(MavrykContextTezedgeOnDiskBackendOptions {
            base_path: "".to_string(),
            startup_check: false,
        }),
...
pub fn context_set_get_commit(
    ...
    // context
    let mut context = initialize_tezedge_context(&MavrykContextTezEdgeStorageConfiguration {
        backend,
    ...
    let hash = context.commit("Mavryk".to_string(), "Genesis".to_string(), 0)?;
...
pub fn context_hash_from_working_tree(
    ...
    let hash = context
        .hash("Mavryk".to_string(), "Genesis".to_string(), 0)
        .unwrap();

    // Created hashes must be commited, this must not panic
    let commit_hash = context
        .commit("Mavryk".to_string(), "Genesis".to_string(), 0)
        .unwrap();
