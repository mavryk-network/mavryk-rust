...
pub use mavryk_encoding_derive::BinWriter;
...
impl BinWriter for Zarith {
    fn bin_write(&self, output: &mut Vec<u8>) -> BinResult {
        ...
        while bits.len() > 0 && !bits[bits.len() - 1] {
            let _ = bits.pop();
        }
        ...
        let mut encoded_bytes = &mut encoding.to_bytes();

        output.append(&mut encoded_bytes);
...
encode_hash!(crypto::hash::Layer2Mv4Hash);
...
#[cfg(test)]
mod test {
    use crate::{enc::BinWriter, types::Zarith};
