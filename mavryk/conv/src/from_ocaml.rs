...
use crate::{
    ...
    OCamlRationalString, OCamlMavrykContextTezedgeOnDiskBackendOptions,
};
...
use super::{
    ...
    OCamlRpcMethod, OCamlMavrykContextTezEdgeStorageConfiguration, OCamlMavrykErrorTrace,
...
use mavryk_api::ffi::{
    ...
    RestoreContextError, RpcArgDesc, RpcMethod, MavrykErrorTrace, MavrykStorageInitError,
    ValidateOperationError, ValidateOperationResponse, ValidateOperationResult, Validated,
};
use mavryk_context_api::{
    ContextKvStoreConfiguration, MavrykContextTezEdgeStorageConfiguration,
...
impl_from_ocaml_record! {
    OCamlMavrykContextTezedgeOnDiskBackendOptions => MavrykContextTezedgeOnDiskBackendOptions {
        base_path: String,
        startup_check: bool,
    }
}
...
impl_from_ocaml_variant! {
    OCamlContextKvStoreConfiguration => ContextKvStoreConfiguration {
        ...
        ContextKvStoreConfiguration::InMem(options: OCamlMavrykContextTezedgeOnDiskBackendOptions),
        ContextKvStoreConfiguration::OnDisk(options: OCamlMavrykContextTezedgeOnDiskBackendOptions),
    }
}

impl_from_ocaml_record! {
    OCamlMavrykContextTezEdgeStorageConfiguration => MavrykContextTezEdgeStorageConfiguration {
...
impl_from_ocaml_record! {
    OCamlMavrykErrorTrace => MavrykErrorTrace {
...
// TODO: move this conversion logic to FfiPath to remove duplication in mavryk/interop
...
impl_from_ocaml_polymorphic_variant! {
    OCamlNodeMessage => NodeMessage {
        ApplyBlockResult(result: Result<OCamlApplyBlockResponse, OCamlMavrykErrorTrace>) =>
            NodeMessage::ApplyBlockResult(result),
        AssertEncodingForProtocolDataResult(result: Result<(), OCamlMavrykErrorTrace>) =>
            NodeMessage::AssertEncodingForProtocolDataResult(result),
        BeginApplicationResult(result: Result<OCamlBeginApplicationResponse, OCamlMavrykErrorTrace>) =>
            NodeMessage::BeginApplicationResult(result),
        BeginConstructionResult(result: Result<OCamlPrevalidatorWrapper, OCamlMavrykErrorTrace>) =>
            NodeMessage::BeginConstructionResult(result),
        PreFilterOperationResult(result: Result<OCamlPreFilterOperationResponse, OCamlMavrykErrorTrace>) =>
            NodeMessage::PreFilterOperationResult(result),
        ValidateOperationResponse(result: Result<OCamlValidateOperationResponse, OCamlMavrykErrorTrace>) =>
            NodeMessage::ValidateOperationResponse(result),
        PreapplyBlockResponse(result: Result<OCamlPreapplyBlockResponse, OCamlMavrykErrorTrace>) =>
        ...
        HelpersPreapplyResponse(result: Result<OCamlHelpersPreapplyResponse, OCamlMavrykErrorTrace>) =>
            NodeMessage::HelpersPreapplyResponse(result),
        ChangeRuntimeConfigurationResult =>
            NodeMessage::ChangeRuntimeConfigurationResult,
        InitProtocolContextResult(result: Result<OCamlInitProtocolContextResult, OCamlMavrykErrorTrace>) =>
        ...
        CommitGenesisResultData(result: Result<OCamlCommitGenesisResult, OCamlMavrykErrorTrace>) =>
            NodeMessage::CommitGenesisResultData(result),
        ComputePathResponse(result: Result<OCamlComputePathResponse, OCamlMavrykErrorTrace>) =>
            NodeMessage::ComputePathResponse(result),
        JsonEncodeApplyBlockResultMetadataResponse(result: Result<String, OCamlMavrykErrorTrace>) =>
            NodeMessage::JsonEncodeApplyBlockResultMetadataResponse(result),
        JsonEncodeApplyBlockOperationsMetadata(result: Result<String, OCamlMavrykErrorTrace>) =>
        ...
        DumpContextResponse(result: Result<OCamlInt, OCamlMavrykErrorTrace>) =>
            NodeMessage::DumpContextResponse(result),
        RestoreContextResponse(result: Result<(), OCamlMavrykErrorTrace>) =>
            NodeMessage::RestoreContextResponse(result),
        IntegrityCheckContextResponse(result: Result<(), OCamlMavrykErrorTrace>) =>
            NodeMessage::IntegrityCheckContextResponse(result),

        ContextGetLatestContextHashesResult(result: Result<OCamlList<OCamlContextHash>, OCamlMavrykErrorTrace>) =>
...
macro_rules! from_ocaml_mavryk_error_trace {
    ($rust_name:ident, $from:ty) => {
        unsafe impl FromOCaml<OCamlMavrykErrorTrace> for $rust_name {
            fn from_ocaml(v: OCaml<OCamlMavrykErrorTrace>) -> Self {
                let trace: MavrykErrorTrace = v.to_rust();
...
from_ocaml_mavryk_error_trace!(ApplyBlockError, mavryk_api::ffi::CallError);
from_ocaml_mavryk_error_trace!(ProtocolDataError);
from_ocaml_mavryk_error_trace!(BeginApplicationError, mavryk_api::ffi::CallError);
from_ocaml_mavryk_error_trace!(BeginConstructionError, mavryk_api::ffi::CallError);
from_ocaml_mavryk_error_trace!(PreFilterOperationError, mavryk_api::ffi::CallError);
from_ocaml_mavryk_error_trace!(ValidateOperationError, mavryk_api::ffi::CallError);
from_ocaml_mavryk_error_trace!(HelpersPreapplyError, mavryk_api::ffi::CallError);
from_ocaml_mavryk_error_trace!(MavrykStorageInitError);
from_ocaml_mavryk_error_trace!(GetDataError);
from_ocaml_mavryk_error_trace!(FfiJsonEncoderError);
from_ocaml_mavryk_error_trace!(ComputePathError, mavryk_api::ffi::CallError);
from_ocaml_mavryk_error_trace!(DumpContextError);
from_ocaml_mavryk_error_trace!(GetLastContextHashesError);
from_ocaml_mavryk_error_trace!(RestoreContextError);
from_ocaml_mavryk_error_trace!(IntegrityCheckContextError);
