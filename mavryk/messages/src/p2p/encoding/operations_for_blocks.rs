...
use mavryk_encoding::nom::NomResult;
use mavryk_encoding::{
...
/// This is calculated from Mavryk limit on that Operation_list_list size:
///
/// `let operation_max_pass = ref (Some 8) (* FIXME: arbitrary *)`
///
/// See https://gitlab.com/tezedge/mavryk/-/blob/master/src/lib_shell/distributed_db_message.ml#L65
///
...
fn bin_write_path_items(
    path_items: &[PathItem],
    out: &mut Vec<u8>,
) -> mavryk_encoding::enc::BinResult {
    match path_items.split_first() {
        None => {
            mavryk_encoding::enc::u8(&0x00, out)?;
        }
        Some((PathItem::Left(left), rest)) => {
            mavryk_encoding::enc::u8(&0xf0, out)?;
            bin_write_path_items(rest, out)?;
            mavryk_encoding::enc::put_bytes(&left.right, out);
        }
        Some((PathItem::Right(right), rest)) => {
            mavryk_encoding::enc::u8(&0x0f, out)?;
            mavryk_encoding::enc::put_bytes(&right.left, out);
            bin_write_path_items(rest, out)?;
        }
    }
    Ok(())
