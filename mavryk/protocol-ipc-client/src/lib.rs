...
use mavryk_api::{environment::MavrykEnvironmentConfiguration, ffi::*};
use mavryk_context_api::{
    ContextKeyOwned, ContextValue, PatchContext, StringTreeObject, MavrykContextStorageConfiguration,
};
...
pub enum ProtocolRunnerError {
    #[error("Failed to spawn mavryk protocol wrapper sub-process: {reason}")]
    SpawnError { reason: String },
    #[error("Timeout when waiting for protocol runner connection socket")]
    SocketTimeout,
    #[error("Failed to terminate/kill mavryk protocol wrapper sub-process, reason: {reason}")]
...
pub struct ProtocolRunnerConfiguration {
    pub runtime_configuration: MavrykRuntimeConfiguration,
    pub environment: MavrykEnvironmentConfiguration,
...
impl ProtocolRunnerConnection {
    ...
    /// Change mavryk runtime configuration
    pub async fn change_runtime_configuration(
        &mut self,
        settings: MavrykRuntimeConfiguration,
    ) -> Result<(), ProtocolServiceError> {
        handle_request!(
            self.io,
    ...
    /// Command mavryk ocaml code to initialize context and protocol.
    ...
    pub async fn genesis_result_data(
        ...
        let mavryk_environment = self.configuration.environment.clone();
        let main_chain_id = mavryk_environment.main_chain_id().map_err(|e| {
            ProtocolServiceError::InvalidDataError {
                message: format!("{:?}", e),
            }
        })?;
        let protocol_hash = mavryk_environment.genesis_protocol().map_err(|e| {
            ProtocolServiceError::InvalidDataError {
                message: format!("{:?}", e),
            }
        })?;
