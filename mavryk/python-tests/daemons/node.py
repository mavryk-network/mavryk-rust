...
class Node:
    """Wrapper for the mavryk-node command.

    This class manages the persistent state of a mavryk-node
    ...
    - mavryk-node identity generate
    - mavryk-node upgrade storage

    mavryk-node run is intended to be used asynchronously and forks a
    subprocess.
    ...
    node.run() # run mavryk-node process
    ...
    def __init__(
        ...
        """Creates a new Popen instance for a mavryk-node, and manages context.
        ...
        if node_dir is None:
            node_dir = tempfile.mkdtemp(prefix='mavryk-node.')
        ...
        if log_levels is not None:
            ...
            new_env['MAVRYK_LOG'] = lwt_log
        # TODO: FIXME
        new_env = os.environ.copy()
        new_env['LD_LIBRARY_PATH'] = paths.MAVRYK_HOME
        self._new_env = new_env
        if self.node_name == 'mavryk-node':
    ...
    def init_config(self):
        ...
        if self.use_tls:
            # We can't create mavryk.crt/mavryk.key here
            # as node_dir has to be empty when we run node_config
            node_config += [
                '--rpc-tls',
                (f'{self.node_dir}/mavryk.crt,' f'{self.node_dir}/mavryk.key'),
            ]
    ...
    def init_id(self):
        ...
        if self.use_tls:
            with open(f'{self.node_dir}/mavryk.crt', 'w+') as file:
                file.write(self.use_tls[0])
            with open(f'{self.node_dir}/mavryk.key', 'w+') as file:
