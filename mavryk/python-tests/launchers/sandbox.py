...
NODE = 'light-node'
CLIENT = 'mavryk-client'
CLIENT_ADMIN = 'mavryk-admin-client'
BAKER = 'mavryk-baker'
ENDORSER = 'mavryk-endorser'
ACCUSER = 'mavryk-accuser'
NODE_LETTER = {
    'T': 'light-node',
    'O': 'mavryk-node',
}
...
node_sequence = (
    NODE_LETTER[l]
    for l in itertools.cycle(os.getenv('MAVRYK_NODE_SEQUENCE', 'T'))
)
...
class Sandbox:
    ...
    def _wrap_path(self, binary: str, branch: str, proto="") -> str:
        ...
        - follows mavryk naming convention for binaries based on protocol
    ...
    def __init__(
        ...
        Args:
            binaries_path (str): path to the binaries (client, node, baker,
                endorser). Typically, this parameter is MAVRYK_HOME.
        ...
        `mavryk-endorser-002-PsYLVpVv` is the endorser for protocol
        `-002-PsYLVpVv`.
    ...
    def register_node(
        ...
        light-node --mavryk-data-dir TMP_DIR
        ...
        if os.environ.get('CONTEXT_MUST_SURVIVE_RESTARTS'):
            ...
            params = params + ['--mavryk-context-storage=irmin']
    ...
    def add_node(
        ...
        Args:
            ...
            use_tls (Tuple[str, str]): couple of filenames containing
                           (mavryk.crt, mavryk.k), default=None
    ...
    def add_baker(
        ...
        Args:
            ...
            proto (str): name of protocol, used to determine the binary to
                         use. E.g. 'alpha` for `mavryk-baker-alpha`.
    ...
    def add_endorser(
        ...
        Args:
            ...
            proto (str): name of protocol, used to determine the binary to
                         use. E.g. 'alpha` for `mavryk-endorser-alpha`.
            params (list): additional parameters
            branch (str): see branch parameter for `add_node()`
        """
        assert node_id in self.nodes, f'No node running with id={node_id}'
    ...
    def add_accuser(
        ...
        Args:
            node_id (int): id of corresponding node
            proto (str): name of protocol, used to determine the binary to
                         use. E.g. 'alpha` for `mavryk-accuser-alpha`.
            branch (str): see branch parameter for `add_node()`
        """
        assert node_id in self.nodes, f'No node running with id={node_id}'
