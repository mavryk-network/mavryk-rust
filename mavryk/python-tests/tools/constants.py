...
def get_parameters(folder: str, network='test') -> dict:
    ...
    params_file = (
        f'{paths.MAVRYK_HOME}/src/{folder}/parameters/'
...
COMPILER = f'{paths.MAVRYK_HOME}/mavryk-protocol-compiler'


# This is the secret key used to activate a protocol from genesis in sandbox
# mode. The corresponding public key is hard-coded in the mavryk node.
...
ALPHA_DAEMON = "alpha"  # mavryk-baker-alpha
...
"""
MAVKIT_NODE_PARAMS = ['--connections', '100', '--synchronisation-threshold', '0']

TEZEDGE_NODE_PARAMS = [
    '--sandbox-patch-context-json-file',
    paths.MAVRYK_HOME + 'sandbox-patch-context.json',
    '--bootstrap-db-path',
    'light-node',
    '--log-format',
    'simple',
    ...
    'false',
    '--protocol-runner',
    paths.MAVRYK_HOME + 'protocol-runner',
    '--peer-thresh-low',
    '250',
    '--peer-thresh-high',
    '500',
    ...
    paths.MAVRYK_HOME + 'sapling-spend.params',
    '--init-sapling-output-params-file',
    paths.MAVRYK_HOME + 'sapling-output.params',
]
...
class NodeParams:
    def __init__(
        self,
        mavkit_params: List[str],
        tezedge_params: List[str],
        more_params: List[str] = [],
    ):
        self._mavkit_params = mavkit_params
    ...
    def __getitem__(self, node_name: str):
        if node_name == 'mavryk-node':
            return self._mavkit_params + self._more_params
...
NODE_PARAMS = NodeParams(
    mavkit_params=MAVKIT_NODE_PARAMS, tezedge_params=TEZEDGE_NODE_PARAMS
)
