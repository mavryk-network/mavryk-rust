...
use mavryk_api::environment::{self, MavrykEnvironment};
use mavryk_api::ffi::{
    InitProtocolContextResult, ProtocolDataError, RustBytes, MavrykRuntimeConfiguration,
    MavrykRuntimeLogLevel,
};
...
fn test_init_protocol_context() {
    ...
    let InitProtocolContextResult {
        genesis_commit_hash,
        supported_protocol_hashes,
    } = prepare_protocol_context(storage_dir, &mavryk_env, false);
