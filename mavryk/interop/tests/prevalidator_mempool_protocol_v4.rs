...
fn test_begin_construction() -> Result<(), anyhow::Error> {
    ...
    let (chain_id, genesis_block_header, ..) = common::init_test_protocol_context(
        ...
        test_data_protocol_v4::mavryk_network(),
    );
...
mod test_data_protocol_v4 {
    ...
    // curl -s https://testnet-mavryk.giganode.io/chains/main/blocks/2/header/raw

    // Encoded block header hashes with:
    // ./mavryk-codec encode alpha.operation.raw from '{ "branch": "BLAuTiHQdSCXXhfDvv417JKsM8bYT5tNsrpmWyJiMrMdYJNuoLP", "data": "" }'
    ...
    // Extracted from https://testnet-mavryk.giganode.io/chains/main/blocks/3/operations/0/0
    // and encoded with `mavryk-codec encode 011-PtHangz2.operation from ./endorsement.json`.
    // JSON:
    // {
