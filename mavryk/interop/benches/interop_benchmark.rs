...
fn init_bench_runtime() {
    ...
    //ffi::change_runtime_configuration(MavrykRuntimeConfiguration {
    //    debug_mode: false,
...
fn apply_block_request_decoded_roundtrip(
    ...
    runtime::execute(move |rt: &mut OCamlRuntime| {
        ...
        let result = mavryk_ffi::apply_block_request_decoded_roundtrip(rt, &request);
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    let _ignored = runtime::execute(move |rt: &mut OCamlRuntime| {
        ...
        mavryk_ffi::setup_benchmark_apply_block_response(rt, &ocaml_response);
    });
