...
const GIT_RELEASE_DISTRIBUTIONS_FILE: &str = "lib_mavryk/libmavryk-ffi-distribution-summary.json";
const LIBMAVRYK_BUILD_NAME: &str = "libmavryk.so";
const ARTIFACTS_DIR: &str = "lib_mavryk/artifacts";
...
const ZCASH_PARAM_SAPLING_FILES_MAVRYK_PATH: &str = "_opam/share/zcash-params";
...
fn get_remote_lib(artifacts: &[Artifact]) -> RemoteFile {
    ...
    let artifact_for_platform = match platform.os_type {
        OSType::OSX => match std::env::consts::ARCH {
            "x86_64" => Some("libmavryk-ffi-macos.dylib.gz"),
            "aarch64" => Some("libmavryk-ffi-macos-m1.dylib.gz"),
            _ => None,
        },
        OSType::Ubuntu => match platform.version.as_str() {
            "16.04" => Some("libmavryk-ffi-ubuntu16.so.gz"),
            "18.04" | "18.10" => Some("libmavryk-ffi-ubuntu18.so.gz"),
            "19.04" | "19.10" => Some("libmavryk-ffi-ubuntu19.so.gz"),
            "20.04" | "20.10" => Some("libmavryk-ffi-ubuntu20.so.gz"),
            "21.04" | "21.10" => Some("libmavryk-ffi-ubuntu21.so.gz"),
            "22.04" => Some("libmavryk-ffi-ubuntu22.so.gz"),
            _ => None,
        },
        OSType::Debian => match platform.version.as_str() {
            "9" => Some("libmavryk-ffi-debian9.so.gz"),
            v if v.starts_with("9.") => Some("libmavryk-ffi-debian9.so.gz"),
            "10" => Some("libmavryk-ffi-debian10.so.gz"),
            v if v.starts_with("10.") => Some("libmavryk-ffi-debian10.so.gz"),
            _ => None,
        },
        OSType::OpenSUSE => match platform.version.as_str() {
            "15.1" | "15.2" => Some("libmavryk-ffi-opensuse15.1.so.gz"),
            v if v.len() == 8 && v.chars().all(char::is_numeric) => {
                Some("libmavryk-ffi-opensuse_tumbleweed.so.gz")
            }
            _ => None,
        },
        OSType::CentOS => match platform.version.chars().next().unwrap() {
            ...
            '7' => Some("libmavryk-ffi-centos7.so.gz"),
            '8' => Some("libmavryk-ffi-centos8.so.gz"),
            _ => None,
        },
        _ => None,
    };
...
fn libmavryk_filename() -> &'static str {
    let platform = current_platform();
    match platform.os_type {
        OSType::OSX => "libmavryk.dylib",
        _ => "libmavryk.so",
    }
}
...
fn run_builder(build_chain: &BuildChain) {
    ...
    let libmavryk_ffi_dst_path = artifacts_path.join(libmavryk_filename());
    ...
    match build_chain {
        BuildChain::Local(mavryk_base_dir) => {
            let mavryk_path = Path::new(&mavryk_base_dir);
            let libmavryk_ffi_src_path = mavryk_path.join(LIBMAVRYK_BUILD_NAME);
            let zcash_params_spend_src_path = mavryk_path
                .join(ZCASH_PARAM_SAPLING_FILES_MAVRYK_PATH)
                .join(ZCASH_PARAM_SAPLING_SPEND_FILE_NAME);
            let zcash_params_output_src_path = mavryk_path
                .join(ZCASH_PARAM_SAPLING_FILES_MAVRYK_PATH)
                .join(ZCASH_PARAM_SAPLING_OUTPUT_FILE_NAME);

            if !mavryk_path.exists() {
                println!(
                    "{} MAVRYK_BASE_DIR={} was not found!",
                    "error".bright_red(),
                    mavryk_base_dir
                );
                panic!()
            ...
            if !libmavryk_ffi_src_path.exists() {
                println!(
                    "{} {} was not found!",
                    "error".bright_red(),
                    libmavryk_ffi_src_path.to_str().unwrap()
                );

                println!();
                println!(
                    "Please build libmavryk-ffi before continuing (see: ./mavryk/interop/README.md)."
                );
                panic!();
            }
            ...
            if !zcash_params_spend_src_path.exists() {
                ...
                println!();
                println!(
                    "Please build Mavryk repo before continuing (see: ./mavryk/interop/README.md)."
                );
                panic!();
            }
            ...
            Command::new("cp")
                .args(&[
                    "-f",
                    libmavryk_ffi_src_path.to_str().unwrap(),
                    libmavryk_ffi_dst_path.to_str().unwrap(),
                ])
                .status()
                .unwrap_or_else(|_| {
                    panic!(
                        "Couldn't copy '{:?}' to '{:?}'",
                        libmavryk_ffi_src_path, libmavryk_ffi_dst_path
                    )
                });
...
fn main() {
    ...
    let mavryk_base_dir = env::var("MAVRYK_BASE_DIR").unwrap_or_else(|_| "".to_owned());
    let build_chain = if mavryk_base_dir.is_empty() {
        BuildChain::Remote
    } else {
        BuildChain::Local(mavryk_base_dir)
    };
    ...
    if bytes_copied == 0 {
        ...
        panic!("Failed to build mavryk_interop artifacts.");
    }

    println!("cargo:rustc-link-search={}", &out_dir);
    println!("cargo:rustc-link-lib=dylib=mavryk");
    println!("cargo:rerun-if-env-changed=MAVRYK_BASE_DIR");
}
