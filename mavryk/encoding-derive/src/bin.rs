...
pub fn generate_bin_write_for_data(data: &DataWithEncoding) -> TokenStream {
    ...
    quote_spanned! {
        ...
        impl mavryk_encoding::enc::BinWriter for #name {
            fn bin_write(&self, out: &mut Vec<u8>) -> mavryk_encoding::enc::BinResult {
                #bin_write(self, out)
            }
        }
    }
...
fn generate_bin_write(encoding: &Encoding) -> TokenStream {
    match encoding {
        ...
        Encoding::Path(path) => {
            quote_spanned!(path.span()=> <#path as mavryk_encoding::enc::BinWriter>::bin_write)
        }
        ...
        Encoding::Zarith(span) => quote_spanned!(*span=> mavryk_encoding::enc::zarith),
        Encoding::MuMav(span) => quote_spanned!(*span=> mavryk_encoding::enc::mumav),
    }
}

fn generate_bytes_bin_write(span: Span) -> TokenStream {
    quote_spanned!(span=> mavryk_encoding::enc::bytes)
}
...
fn generage_primitive_bin_write(kind: PrimitiveEncoding, span: Span) -> TokenStream {
    match kind {
        ...
        PrimitiveEncoding::Bool => quote_spanned!(span=> mavryk_encoding::enc::boolean),
    }
}

fn generate_number_bin_write(num: &str, span: Span) -> TokenStream {
    let ty = syn::Ident::new(num, span);
    quote_spanned!(span=> mavryk_encoding::enc::#ty)
}
    ...
    quote_spanned! {
        encoding.name.span()=>
            (|data: &Self, out: &mut Vec<u8>| {
                #(
                    mavryk_encoding::enc::field(#field_name, #field_bin_write)(&data.#field, out)?;
                )*
                Ok(())
            })
    }
...
fn generate_enum_bin_write(encoding: &EnumEncoding) -> TokenStream {
    ...
    let tag_serialize = quote_spanned!(encoding.tag_type.span()=> mavryk_encoding::enc::#tag_type);
    ...
    match &tag.encoding {
        Encoding::Unit => {
            quote_spanned!(tag_name.span()=>
                           #enum_name::#tag_name => mavryk_encoding::enc::variant(#name, #tag_encoding)(&#tag_id, out)
            )
        }
        encoding => {
            let bin_write = generate_bin_write(encoding);
            quote_spanned!(tag_name.span()=>
                           #enum_name::#tag_name(inner) => mavryk_encoding::enc::variant_with_field(#name, #tag_encoding, #bin_write)(&#tag_id, inner, out)
            )
        }
    }
}
...
fn generate_string_bin_write(size: &Option<syn::Expr>, span: Span) -> TokenStream {
    size.as_ref().map_or_else(
        || quote_spanned!(span=> mavryk_encoding::enc::string),
        |size| quote_spanned!(span=> mavryk_encoding::enc::bounded_string(#size)),
    )
}

fn generate_optional_field_bin_write(encoding: &Encoding, span: Span) -> TokenStream {
    let bin_write = generate_bin_write(encoding);
    quote_spanned!(span=> mavryk_encoding::enc::optional_field(#bin_write))
}
...
fn generate_list_bin_write(
    ...
    let bin_write = generate_bin_write(encoding);
    size.as_ref().map_or_else(
        || quote_spanned!(span=> mavryk_encoding::enc::list(#bin_write)),
        |size| quote_spanned!(span=> mavryk_encoding::enc::bounded_list(#size, #bin_write)),
    )
}

fn generate_sized_bin_write(size: &syn::Expr, encoding: &Encoding, span: Span) -> TokenStream {
    let bin_write = generate_bin_write(encoding);
    quote_spanned!(span=> mavryk_encoding::enc::sized(#size, #bin_write))
}

fn generate_bounded_bin_write(size: &syn::Expr, encoding: &Encoding, span: Span) -> TokenStream {
    let bin_write = generate_bin_write(encoding);
    quote_spanned!(span=> mavryk_encoding::enc::bounded(#size, #bin_write))
}

fn generate_short_dynamic_bin_write(encoding: &Encoding, span: Span) -> TokenStream {
    let bin_write = generate_bin_write(encoding);
    quote_spanned!(span=> mavryk_encoding::enc::short_dynamic(#bin_write))
}
...
fn generate_dynamic_bin_write(
    ...
    let bin_write = generate_bin_write(encoding);
    size.as_ref().map_or_else(
        || quote_spanned!(span=> mavryk_encoding::enc::dynamic(#bin_write)),
        |size| quote_spanned!(span=> mavryk_encoding::enc::bounded_dynamic(#size, #bin_write)),
    )
}
