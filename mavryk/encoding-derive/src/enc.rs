...
pub fn generate_encoding_for_data(data: &DataWithEncoding) -> TokenStream {
    ...
    quote_spanned! {data.name.span()=>
        impl mavryk_encoding::encoding::HasEncoding for #name {
            fn encoding() -> mavryk_encoding::encoding::Encoding {
                #encoding
            }
        }
    }
...
pub(crate) fn generate_encoding(encoding: &Encoding) -> TokenStream {
    match encoding {
        Encoding::Unit => quote!(mavryk_encoding::encoding::Encoding::Unit),
        Encoding::Primitive(primitive, span) => generage_primitive_encoding(*primitive, *span),
        Encoding::Bytes(span) => quote_spanned!(*span=> mavryk_encoding::encoding::Encoding::Bytes),
        Encoding::Path(path) => {
            quote_spanned!(path.span()=> #[allow(clippy::redundant_clone)]<#path as mavryk_encoding::encoding::HasEncoding>::encoding().clone())
        }
        ...
        Encoding::Zarith(span) => quote_spanned!(*span=> mavryk_encoding::encoding::Encoding::Z),
        Encoding::MuMav(span) => quote_spanned!(*span=> mavryk_encoding::encoding::Encoding::Mumav),
    }
}

fn generage_primitive_encoding(kind: PrimitiveEncoding, span: Span) -> TokenStream {
    let ident = kind.make_ident(span);
    quote_spanned!(ident.span()=> mavryk_encoding::encoding::Encoding::#ident)
}
...
fn generate_struct_encoding(encoding: &StructEncoding) -> TokenStream {
    ...
    quote_spanned! { encoding.name.span()=>
        mavryk_encoding::encoding::Encoding::Obj(#name_str, vec![
            #(#fields_encoding),*
        ])
    }
}
...
fn generate_field_encoding(field: &FieldEncoding) -> Option<TokenStream> {
    if let FieldKind::Encoded(encoding) = &field.kind {
        ...
        Some(
            quote_spanned!(field.name.span()=> mavryk_encoding::encoding::Field::new(#name, #encoding)),
        )
    } else {
        None
    }
...
fn generate_enum_encoding(encoding: &EnumEncoding) -> TokenStream {
    ...
    quote_spanned! { tag_type.span()=>
        mavryk_encoding::encoding::Encoding::Tags(
            std::mem::size_of::<#tag_type>(),
            mavryk_encoding::encoding::TagMap::new(vec![
                #(#tags_encoding),*
            ])
        )
    }
...
fn generate_tag_encoding(tag: &Tag) -> TokenStream {
    ...
    quote_spanned!(tag.name.span()=> mavryk_encoding::encoding::Tag::new(#id, #name, #encoding))
}

fn generate_string_encoding(size: &Option<syn::Expr>, span: Span) -> TokenStream {
    size.as_ref().map_or_else(
        || quote_spanned!(span=> mavryk_encoding::encoding::Encoding::String),
        |size| quote_spanned!(span=> mavryk_encoding::encoding::Encoding::BoundedString(#size)),
    )
}
...
fn generate_list_encoding<'a>(
    ...
    size.as_ref().map_or_else(|| quote_spanned!(span=> mavryk_encoding::encoding::Encoding::List(Box::new(#encoding))), |size| quote_spanned!(span=> mavryk_encoding::encoding::Encoding::BoundedList(#size, Box::new(#encoding))))
}

fn generate_optional_field_encoding(encoding: &Encoding, span: Span) -> TokenStream {
    let encoding = generate_encoding(encoding);
    quote_spanned!(span=> mavryk_encoding::encoding::Encoding::OptionalField(Box::new(#encoding)))
}
...
fn generate_sized_encoding<'a>(
    ...
    let encoding = generate_encoding(encoding);
    quote_spanned!(span=> mavryk_encoding::encoding::Encoding::Sized(#size, Box::new(#encoding)))
}
...
fn generate_bounded_encoding<'a>(
    ...
    let encoding = generate_encoding(encoding);
    quote_spanned!(span=> mavryk_encoding::encoding::Encoding::Bounded(#size, Box::new(#encoding)))
}

fn generate_short_dynamic_encoding(encoding: &Encoding, span: Span) -> TokenStream {
    let encoding = generate_encoding(encoding);
    quote_spanned!(span=> mavryk_encoding::encoding::Encoding::ShortDynamic(Box::new(#encoding)))
}
...
fn generate_dynamic_encoding<'a>(
    ...
    let encoding = generate_encoding(encoding);
    size.as_ref().map_or_else(
        || quote_spanned!(span=> mavryk_encoding::encoding::Encoding::Dynamic(Box::new(#encoding))),
        |size| quote_spanned!(span=> mavryk_encoding::encoding::Encoding::BoundedDynamic(#size, Box::new(#encoding))))
}
