...
pub fn generate_nom_read_for_data(data: &DataWithEncoding) -> TokenStream {
    ...
    quote_spanned! {
        ...
        impl mavryk_encoding::nom::NomReader for #name {
            fn nom_read(bytes: &[u8]) -> mavryk_encoding::nom::NomResult<Self> {
                #nom_read(bytes)
            }
        }
    }
...
fn generate_nom_read(encoding: &Encoding) -> TokenStream {
    match encoding {
        ...
        Encoding::Path(path) => {
            quote_spanned!(path.span()=> <#path as mavryk_encoding::nom::NomReader>::nom_read)
        }
        ...
        Encoding::Zarith(span) => quote_spanned!(*span=> mavryk_encoding::nom::zarith),
        Encoding::MuMav(span) => quote_spanned!(*span=> mavryk_encoding::nom::mumav),
    }
}
...
fn generage_primitive_nom_read(kind: PrimitiveEncoding, span: Span) -> TokenStream {
    match kind {
        ...
        PrimitiveEncoding::Bool => quote_spanned!(span=> mavryk_encoding::nom::boolean),
    }
}
...
fn generate_bytes_nom_read(span: Span) -> TokenStream {
    quote_spanned!(span=> mavryk_encoding::nom::bytes)
}
...
fn generate_struct_one_field_nom_read(encoding: &StructEncoding) -> TokenStream {
    ...
    quote_spanned!(encoding.name.span()=> nom::combinator::map(mavryk_encoding::nom::field(#field_name_str, #field_nom_read), |#field_name| #name { #field_name }))
}
    ...
    if let Some(hash_field) = hash.first() {
        ...
        quote_spanned! {
            hash_field.name.span()=>
                nom::combinator::map(
                    mavryk_encoding::nom::hashed(
                        nom::sequence::tuple((
                            #(mavryk_encoding::nom::field(#field_name, #field_nom_read)),*
                        ))
                    ),
                    |((#(#field2),*), #hash_name)| {
        ...
        quote_spanned! {
            hash_field.name.span()=>
                nom::combinator::map(
                    mavryk_encoding::nom::hashed(
                        (|input| {
                            #(let (input, #field1) = mavryk_encoding::nom::field(#field_name, #field_nom_read)(input)?;)*
                            Ok((input, (#(#field2),* )))
                        })
                    ),
                    |((#(#field3),*), #hash_name)| {
        ...
        tag_type.span()=>
            (|input| {
                ...
                )*
                {
                    return Err(
                        nom::Err::Error(
                            mavryk_encoding::nom::error::DecodeError::#unknown_tag_error(
                                input,
                                format!("0x{:.2X}", tag)
                            )
                        )
...
fn generate_tag_nom_read<'a>(tag: &Tag<'a>, enum_name: &syn::Ident) -> TokenStream {
    ...
    match &tag.encoding {
        ...
        encoding => {
            ...
            quote_spanned!(tag_name.span()=> nom::combinator::map(mavryk_encoding::nom::variant(#name, #nom_read), #enum_name::#tag_name))
        }
    }
}
...
fn generate_string_nom_read(size: &Option<syn::Expr>, span: Span) -> TokenStream {
    size.as_ref().map_or_else(
        || quote_spanned!(span=> mavryk_encoding::nom::string),
        |size| quote_spanned!(span=> mavryk_encoding::nom::bounded_string(#size)),
    )
}

fn generate_optional_field_nom_read(encoding: &Encoding, span: Span) -> TokenStream {
    let nom_read = generate_nom_read(encoding);
    quote_spanned!(span=> mavryk_encoding::nom::optional_field(#nom_read))
}
...
fn generate_list_nom_read(
    ...
    let nom_read = generate_nom_read(encoding);
    size.as_ref().map_or_else(
        || quote_spanned!(span=> mavryk_encoding::nom::list(#nom_read)),
        |size| quote_spanned!(span=> mavryk_encoding::nom::bounded_list(#size, #nom_read)),
    )
}

fn generate_sized_nom_read(size: &syn::Expr, encoding: &Encoding, span: Span) -> TokenStream {
    let nom_read = generate_nom_read(encoding);
    quote_spanned!(span=> mavryk_encoding::nom::sized(#size, #nom_read))
}

fn generate_bounded_nom_read(size: &syn::Expr, encoding: &Encoding, span: Span) -> TokenStream {
    let nom_read = generate_nom_read(encoding);
    quote_spanned!(span=> mavryk_encoding::nom::bounded(#size, #nom_read))
}

fn generate_short_dynamic_nom_read(encoding: &Encoding, span: Span) -> TokenStream {
    let nom_read = generate_nom_read(encoding);
    quote_spanned!(span=> mavryk_encoding::nom::short_dynamic(#nom_read))
}
...
fn generate_dynamic_nom_read(
    ...
    let nom_read = generate_nom_read(encoding);
    size.as_ref().map_or_else(
        || quote_spanned!(span=> mavryk_encoding::nom::dynamic(#nom_read)),
        |size| quote_spanned!(span=> mavryk_encoding::nom::bounded_dynamic(#size, #nom_read)),
    )
}
