...
/// Not currently being used, but the mode is available in the mavryk protocol.
pub type Message<'a> = (&'a [u8], &'a PublicKeyBls);

/// Basic `dst` parameter for `blst`
///
/// Same constant as used for `Basic` in `bls12-381` ocaml package used in mavryk.
#[allow(dead_code)]
...
///
/// Same constant as used for verify `Pop` in `bls12-381` ocaml package used in mavryk.
/// Not currently being used, but the mode is available in the mavryk protocol.
#[allow(dead_code)]
...
mod tests {
    ...
    fn ensure_public_key_hashing_correct() {
        ...
        let run_test = |(pk_b58, mv4_b58): (&str, &str)| {
            ...
            let mv4 = pk.pk_hash().expect("PublicKey hashable");
